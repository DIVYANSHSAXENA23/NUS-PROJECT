...
# ---------- 3. FEATURE SET & LABEL ----------
X = df[
    ["distance", "distance_change", "mag_magnitude"]
]
...
def predict_vibration(distance, prev_distance, mag_x, mag_y, mag_z):
    """
    ...
    ----------
    distance : float
        Current distance measurement (same scale as training, e.g. cm).
    prev_distance : float
        Previous distance measurement used to compute distance_change.
    mag_x, mag_y, mag_z : float
        Raw magnetometer readings on each axis.
    """
    ...
    sample = pd.DataFrame(
        [[distance, distance_change, mag_mag]],
        columns=[
            "distance",
            "distance_change",
            "mag_magnitude",
